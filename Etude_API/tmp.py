...
class Analyse:
    ...
    def surrogatePC(self, strg='Quad', degree=5):
        ...
        if (self.strg == 'Quad'):
            ...
            pc_predictor = batman.surrogate.SurrogateModel('LS', self.corners, strategy='LS', degree=self.degree, distributions=self.dists_ot)
            pc_predictor.fit(self.x_train, self.y_train)
...
A=Analyse('test',init_size=10)
A.test()
# A.surrogateKriging()

# A.surrogatePC(degree=2,strg='LS')
