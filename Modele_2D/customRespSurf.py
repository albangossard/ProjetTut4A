...
for gamme in list_gamme:
    ...
    for choice in list_choice:
        ...
        if gamme==-1:
            S=Substitut('sensAnalysis_'+distribName+'_choice='+str(choice),x_train,y_train,corners=corners, dists=dists)
        else:
            S=Substitut('sensAnalysis_'+distribName+'_gamme='+str(gamme)+'_choice='+str(choice),x_train,y_train,corners=corners, dists=dists)
        S.buildK()
        S.analysisK()
        ...
        if gamme==-1:
            plt.savefig('sensAnalysis_'+distribName+'_choice='+str(choice)+'/custom_resp_surface_krig0.png',dpi=200)
        else:
            plt.savefig('sensAnalysis_'+distribName+'_gamme='+str(gamme)+'_choice='+str(choice)+'/custom_resp_surface_krig0.png',dpi=200)
        # plt.show()
        plt.clf()